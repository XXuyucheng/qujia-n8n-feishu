...
**方案**：新增 `feishu-files-generation` 服务（模式同 `pdf-parser`、`feishu-field-parser`），封装「复制模板 + 块遍历替换 + 上传/回链」。

```text
n8n Webhook
  → HTTP 获取记录 + feishu-field-parser 解析
  → POST feishu-files-generation:8030/api/generate
...
n8n
  → 获取记录数据
  → POST feishu-files-generation /api/generate-docx
       读取本地 template.docx
       docxtpl 渲染 {{甲方}} {{金额}}
       upload_all 到飞书文件夹
  → 回写飞书
...
| 性能 | 高。本地渲染，仅上传一步走网络 |
| 与现有架构契合 | 高。仍可用 feishu-files-generation 服务 |
...
| `feishu-files-generation` | **是** | 复制模板、填充、返回文档 URL |

### 6.2 feishu-files-generation 建议 API
...
```python
# 伪代码 — 生产实现在 feishu-files-generation 服务中
...
| 9 | IF | 可生成? | 缺字段则标记失败 |
| 10 | HTTP Request | 调用 feishu-files-generation | `POST /api/generate` |
...
模板 token 不写在表中，统一维护在 `feishu-files-generation/config.yaml`，便于版本管理。
...
- [ ] 创建 `feishu-files-generation` 服务（FastAPI + Docker）
...
```text
    ...
    提取报价基础数据      数据→模板→云文档       云文档/PDF→识别字段
          │                   │                   │
          ▼                   ▼                   ▼
    （待完善输出）        feishu-files-generation      pdf-parser
```
...
2. **推荐路径**：**路径 B（feishu-files-generation 服务 + Block API）** 最适合当前项目架构；复杂模板可降级到**路径 C（本地 DOCX）**。
3. **不推荐**：路径 E（AI 生成合同正文）因法律风险和格式不可控，仅可作为辅助。
4. **下一步**：按第十一章 Phase 0-1 做 POC，验证模板 copy 和 5 个字段替换后，再决定是否创建 `feishu-files-generation` 服务。
