...
> 状态：规划 + 进度跟踪  
> 日期：2026-07-16（进度更新：2026-07-16）  
> 起点：现有 `supplier-bot`（供应商多轮录入）+ `feishu-listener` 对话应用长连接  
...
- 配置目录：[`supplier-bot/config/`](supplier-bot/config/)
  - [`app.yaml`](supplier-bot/config/app.yaml) — 全局 TTL / 口令默认 / 路由
  - [`skills/supplier.yaml`](supplier-bot/config/skills/supplier.yaml) — 供应商技能
- 模块：`config_loader.py`、`router.py`；`dialog.py` 文案/查重/组合必填均读 skill
- 兼容：容器名仍为 `supplier-bot`；改 YAML 后 `docker compose restart supplier-bot`

---

## 1. 为什么要扩展

当前 `supplier-bot` 已验证一条完整闭环：
...
**备注：** 银行简称模糊匹配仍在 `validator.py` 代码内；选项列表与文案已全部 YAML 化。服务目录名暂仍为 `supplier-bot`，多 skill 后再考虑更名 `feishu-chat-bot`。
...
1. **服务演进方式：** 原地把 `supplier-bot` 升级为平台，还是新建 `feishu-chat-bot` 并逐步迁移（推荐：**原地升级目录与模块，保留容器名兼容一段时间**）。  
...
| 现有模块 | 平台中的归宿 |
| --- | --- |
| `supplier-bot/dialog.py` | Form Engine（去业务硬编码） |
