...
bill-parser/          # 微信个人账单解析
bill-parser/n8n/      # 账单入账工作流模板（导入 n8n）
```
...
- 常用健康检查：`http://localhost:8000/health`、`http://localhost:8050/health`、`http://localhost:8010/health` 等（见各服务 README）
...
- 各服务细节见对应目录下的 `README.md`（`pdf-parser/`、`bill-parser/`、`feishu-listener/`、`feishu-bot/` 等）。
